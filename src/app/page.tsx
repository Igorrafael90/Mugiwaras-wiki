...
export default function Home() {
  ...
  return (
    <main className="grid grid-cols-1 w-full">
      <input type="search" value={search} onChange={(e) => setsearch(e.target.value)} className="w-1/2 h-8 mx-auto mt-2 bg-white rounded-md border-black border-1" />
      ...
      <section className="w-full max-w-5xl mx-auto px-6 grid grid-cols-3 gap-8 mt-9">
        {trips.map((trip) => (
          <Link href={`/person/${trip.id}`} key={trip.Name} className="block">
            <div className="bg-neutral-900 h-80 rounded-md p-8 text-gray-300 border-gray-900 border shadow-black shadow-lg hover:scale-110 hover:-translate-y-4 hover:border-red-500">
